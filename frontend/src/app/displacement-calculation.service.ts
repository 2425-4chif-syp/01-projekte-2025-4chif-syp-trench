...
export class DisplacementService {
  ...
  calculateBranchData(branches: { sensors: number[] }[], numberOfBranches: number): { x: number; y: number }[] {
    return branches.map((branch, index) => {
      ...
      // Calculate the degree of the branch using the formula: (2π / numberOfBranches) * index - π/2
      const degree = (2 * Math.PI / numberOfBranches) * index - Math.PI / 2;

      // Calculate the x and y values
      const x = Math.cos(degree) * average;
      const y = Math.sin(degree) * average;
